...
int main() {
    ...
    wcscat_s(exePath, MAX_PATH, L"random_bg.png");
    ...
    // Save the image without CLSID
    CLSID pngClsid;
    UINT numEncoders = 0;
    UINT size = 0;
    GetImageEncodersSize(&numEncoders, &size);
    if (size == 0) {
        OutputDebugStringW(L"Failed to get image encoder size!\n");
    } else {
        ...
        if (pImageCodecInfo) {
            ...
            Status saveStatus = bitmap.Save(exePath, &pngClsid, NULL);
            if (saveStatus != Ok) {
                OutputDebugStringW(L"Failed to save bitmap! Check permissions or path.\n");
            } else {
                OutputDebugStringW(L"Image saved successfully as PNG!\n");
            }
        }
    }

    // Set wallpaper using registry (Windows 7, 10, 11 compatible~!)
    HKEY hKey;
    LONG result = RegOpenKeyExW(HKEY_CURRENT_USER, L"Control Panel\\Desktop", 0, KEY_SET_VALUE, &hKey);
    if (result == ERROR_SUCCESS) {
        result = RegSetValueExW(hKey, L"WallPaper", 0, REG_SZ, (BYTE*)exePath, (wcslen(exePath) + 1) * sizeof(wchar_t));
        if (result == ERROR_SUCCESS) {
            // Refresh the desktop to apply the new wallpaper
            SystemParametersInfoW(SPI_SETDESKWALLPAPER, 0, (void*)exePath, SPIF_UPDATEINIFILE | SPIF_SENDCHANGE);
            OutputDebugStringW(L"Wallpaper set successfully via registry!\n");
        } else {
            wchar_t errorMsg[32];
            wsprintfW(errorMsg, L"RegSetValueExW failed: %d\n", result);
            OutputDebugStringW(errorMsg);
        }
        RegCloseKey(hKey);
    } else {
        wchar_t errorMsg[32];
        wsprintfW(errorMsg, L"RegOpenKeyExW failed: %d\n", result);
        OutputDebugStringW(errorMsg);
    }
